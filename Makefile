...
test:
	docker-compose run --service-ports web pipenv run odoo -c /odoo-dev/addon/config/odoo.conf -u library_website --test-enable --stop
...
install:
	docker-compose run --service-ports web pipenv run odoo -c /odoo-dev/addon/config/odoo.conf -i library_website --stop
